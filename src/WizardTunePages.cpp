...
void DrawAimingTab(WizardState& s) {
    ...
    if (aimInputs.HasSeparateInput()) {
        ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.6f, 1.0f), "Mode: Independent Aim & Steer");
        ImGui::TextWrapped("The flight stick controls ship rotation directly. Bound analog or directional digital aim inputs independently drive the weapon reticle. Clear those inputs to switch to Aim-Driven Steering.");
    } else {
        ImGui::TextColored(ImVec4(1.0f, 0.85f, 0.3f, 1.0f), "Mode: Aim-Driven Steering");
        ImGui::TextWrapped("The flight stick drives both the aiming reticle and ship steering through the mouse accumulator pathway. Bind an analog or directional digital aim input below to switch to Independent Aim & Steer.");
        ImGui::Spacing();
    ...
    ImGui::TextWrapped("Bind a button to toggle between Aim-Driven Steering and Independent Aim at runtime. Only useful when an analog or directional digital aim input is bound.");
    ImGui::Spacing();
    ImGui::PushID(7005);
...
void DrawCameraLookTab(WizardState& s) {
    (void)s;
    ImGui::TextWrapped(
        "Camera pose, OpenTrack, recenter/toggle bindings, joystick look axes, and filtering are owned by the standalone Absolute Head Tracking module.");
    ImGui::Spacing();
    ImGui::TextDisabled(
        "This legacy HOTAS fallback is read-only for those settings and will preserve existing HeadTracking keys without rewriting them.");
}
